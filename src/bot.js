        ...
        .then((data) => {
            ...
            for (var i = 0; i < data["Han"].length; i++) {
                inlineQueryString += "{" +
                    ...
                    "\"message_text\": " + "\"" + "[" + data["Han"][i].name + "]\\n" + (data["Han"][i].temp.trim() === "정보없음" ? data["Han"][i].temp.trim() : data["Han"][i].temp.trim() + "℃") + "\\n" + "(" + data.date.substring(0, data.date.length - 4) + "년 " + data.date.substring(data.date.length - 4, data.date.length - 2) + "월 " + data.date.substring(data.date.length - 2, data.date.length) + "일" + ")" + "\"";
